...
export interface ZilPayWallet {
  ...
  verify: Function;
  observableAccount: (...args: string[]) => ObservableAccount;
  observableNetwork: (...args: string[]) => ObservableNetwork;
  observableBlock: (...args: string[]) => ObservableBlock;
  observableTransaction: (...args: string[]) => ObservableTransaction;
...
export interface ZilPayTransactions {
  ...
  new: (
    transactionParams: ZilPayTransactionProps,
    toDs: boolean
  ) => ZilPayTransaction;
}
...
export interface ZilPayBlockchain {
  ...
  >;
  getContractAddressFromTransactionID: (transactionId: string) => Promise<
    ZilPayBlockchainPayload & {
      result: string;
    }
  >;
  getCurrentDSEpoch: () => Promise<
    ZilPayBlockchainPayload & {
      result: string;
    }
  >;
  getCurrentMiniEpoch: (transactionId: string) => Promise<
    ZilPayBlockchainPayload & {
      result: string;
    }
  >;
  getDSBlock: (blockNum: number) => Promise<
    ZilPayBlockchainPayload & {
      result: ZilPayBlockchainDSBlock;
    }
  >;
  getDSBlockListing: (max: number) => Promise<
    ZilPayBlockchainPayload & {
      result: {
        data: {
          BlockNum: number;
  ...
  >;
  getDSBlockRate: () => Promise<
    ZilPayBlockchainPayload & {
      result: number;
    }
  >;
  getLatestDSBlock: () => Promise<
    ZilPayBlockchainPayload & {
      result: ZilPayBlockchainDSBlock;
    }
  >;
  getMinimumGasPrice: () => Promise<
    ZilPayBlockchainPayload & {
      result: string;
    }
  >;
  createTransaction: (
    tansaction: ZilPayTransaction
  ) => Promise<ZilPayTransactionProps & any>;
}
...
export interface ZilPayContract {
  ...
  call: (
    transaction: string,
    params: ZilPayParam[],
    config: { amount: any; gasPrice: any; gasLimit: any },
    toDs: boolean
  ) => Promise<ZilPayContractCall>;
  deploy: (
    config: { amount: any; gasPrice: any; gasLimit: any },
    toDs: boolean
  ) => Promise<[ZilPayTransaction, ZilPayContract]>;
